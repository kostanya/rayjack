...
void Camera::render(const Hittable& world) {
    ...
    std::unique_ptr<uint8_t[]> imageData = std::make_unique<uint8_t[]>(
        static_cast<size_t>(imageWidth) * m_imageHeight * 3);
    ...
    stbi_write_png("png_test.png", imageWidth, m_imageHeight, m_channel,
                   imageData.get(), imageWidth * m_channel);

    // You have to use 3 comp for complete jpg file. If not, the image will
    // be grayscale or nothing. stbi_write_jpg("jpg_test.jpg", imageWidth,
    // imageHeight, 3, imageData.get(), 100);
}
...
void Camera::initialize() {
    ...
    float viewportWidth =
        viewportHeight * (static_cast<float>(imageWidth) / m_imageHeight);
    ...
    point3 viewportUpperLeft = m_center - vec3(0.0f, 0.0f, focalLength) -
                               viewportU / 2.0f - viewportV / 2.0f;
    m_startPixelLoc =
        viewportUpperLeft + 0.5f * (m_pixelDeltaU + m_pixelDeltaV);
}

Ray Camera::getRay(int i, int j) const {
    // Get randomly sampled camera ray for the pixel at location [i, j]
    point3 pixelCenter = m_startPixelLoc +
                         (static_cast<float>(i) * m_pixelDeltaU) +
                         (static_cast<float>(j) * m_pixelDeltaV);
